...
public class Imgupload {
	...
	public @ResponseBody LinkedList<FileMeta> upload(MultipartHttpServletRequest request,
		...
		while (itr.hasNext()) {
			...
			fileMeta.setFileSize(mpf.getSize() < 1024 ? mpf.getSize() + " B" : mpf.getSize() / 1024 + " KB");
			...
			try {
				...
				// D:/temp/files" exists)
				FileCopyUtils.copy(mpf.getBytes(), new FileOutputStream("D:/temp/files/" + UUID.randomUUID() + "." + mpf.getContentType().split("/")[1]));
